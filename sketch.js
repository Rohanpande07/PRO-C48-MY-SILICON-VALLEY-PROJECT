...
var next,forward,yes,no,home,site,retry,overview,details,size,exception,send,end,communicate;
...
var speechCount = 0;
var delayCount = 0;
var isChecked,assembled,h,planet,canvas,edges,landed,win,RocketPosition;
var click,bgs,winS;

function preload(){
  click = loadSound("sounds/click.wav");
  winS = loadSound("sounds/win.wav");
...
function setup() {
 ...
 exception = document.getElementById("new");
...
function draw() {
  h = document.getElementById("sl").value;
  bgs.setVolume(h/200);
  
  click.setVolume(h/100);
  background(bg);
  drawSprites();
  form.display();
  home.show();
 ...
 if(gameState === "tutorial"){
   ...
   rect(1110,70,200,30);
   fill(rgb(0,255,0));
   rect(1110,70,RocketPosition,30);
   ...
   if(speechState === 29){
   speech(29,"Beware of the obstacles.. and Your destination at a distance of 50000 km.");
   speechBubble.x = 200;
   speechBubble.y = 530;
 ...
 if(rocket.visible === true && gameState === "start"){
   ...
   if(planet.y === -210){
       ...
       tutorial.style.display  = "inline-block";
       tutorial.style.top = "450px";
       speech(31,"Here's your destination . . . . . 'KEPLER - 32d'. Press enter to land.");
       if(keyCode === 13){
         gameState = "landed";
         nxt();
       }
  ...
  if(gameState === "landed"){
    ...
    if(details.disabled === true){
      details.style.backgroundColor = "#2a4d52";
      details.style.cursor = "not-allowed";
    } else {
      details.style.backgroundColor = "rgba(9, 33, 248, 0.6)";
      details.style.cursor = "pointer";
    }
    if(size.disabled === true){
      size.style.backgroundColor = "#2a4d52";
      size.style.cursor = "not-allowed";
    } else {
      size.style.backgroundColor = "rgba(9, 33, 248, 0.6)";
      size.style.cursor = "pointer";
    }
    if(exception.disabled === true){
      exception.style.backgroundColor = "#2a4d52";
      exception.style.cursor = "not-allowed";
    } else {
      exception.style.backgroundColor = "rgba(9, 33, 248, 0.6)";
      exception.style.cursor = "pointer";
    }
    if(send.disabled === true){
      send.style.backgroundColor = "#2a4d52";
      send.style.cursor = "not-allowed";
    } else {
      send.style.backgroundColor = "rgba(9, 33, 248, 0.6)";
      send.style.cursor = "pointer";
    }
    if(end.disabled === true){
      end.style.backgroundColor = "#2a4d52";
      end.style.cursor = "not-allowed";
    } else {
      end.style.backgroundColor = "rgba(9, 33, 248, 0.6)";
    ...
    if(speechState === 32){
     ...
     speech(32,"Now you are landed on KEPLER - 32d. As the commander of this mission, try to explore this planet.");
